...
export default function UserProfile() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                if (userData.id) {
                    const response = await axios.get(`http://localhost:4000/user/${userData.id}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });

                    const updatedUserData = response.data?.user?.[0] || response.data?.user || response.data;
    ...
    const handleSaveProfile = async () => {
        try {
            ...
            const response = await axios.put(`http://localhost:4000/user/${user.id}`, user, {
                headers: {
                    'Authorization': `Bearer ${token}`,
        ...
        return (
            <HelmetProvider>
                <div>
                    ...
                    <Header />
                    <Container fluid className="bg-light d-flex justify-content-center align-items-center" style={{ minHeight: "80vh" }}>
        ...
        <HelmetProvider>
            <div>
                <Helmet>
                    ...
                    <meta name="description" content={`Hồ sơ của ${user.fullname || user.name} - ${user.profession || 'Freelancer'}`} />
                </Helmet>

                <Header />

                <Container fluid className="bg-light py-5" style={{ minHeight: "80vh" }}>
                    <Container>
                                    ...
                                    <Row className="mb-4">
                                        ...
                                        <Col md={6}>
                                            <div className="profile-header">
                                                <h2 className="mb-2 text-primary">{user.fullname || user.name || "Tên người dùng"}</h2>
                                                <p className="text-muted h5 mb-3">{user.profession || "Freelancer"}</p>
                                                <div className="mb-3">
                                                    <span className="badge bg-success me-2">
                                                        {user.rating || "5.0"} ({user.reviews || "15"} đánh giá)
                                                    </span>
                                        ...
                                        <Col md={2} className="text-end">
                                            {isEditing ? (
                                                <div>
                                                    <Button variant="success" size="sm" className="mb-2 w-100" onClick={handleSaveProfile}>
                                                        Lưu
                                                    </Button>
                                                    <Button variant="secondary" size="sm" className="w-100" onClick={handleEditToggle}>
                                                        Hủy
                                                    </Button>
                                                </div>
                                            ) : (
                                                        ...
                                                        <Card className="border-0 shadow-sm">
                                                            <Card.Body>
                                                                <Row className="mb-3">
                                                                    ...
                                                                    <Col md={8}>
                                                                        <p className="mb-0">{user.fullname || user.name || "Chưa cập nhật"}</p>
                                                                    </Col>
                                                                </Row>
                                                                ...
                                                                <Row className="mb-3">
                                                                    ...
                                                                    <Col md={8}>
                                                                        <p className="mb-0">{user.profession || "Freelancer"}</p>
                                                                    </Col>
                                                                </Row>
                                                                ...
                                                                <Row className="mb-3">
                                                                    ...
                                                                    <Col md={8}>
                                                                        <p className="mb-0">{user.bio || "Chưa có thông tin giới thiệu"}</p>
                                                                    </Col>
                                                                </Row>
                                                            </Card.Body>
                                                        </Card>
