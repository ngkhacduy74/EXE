...
const PostDetail = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        // TODO: xác định user đã like/favorite chưa nếu có auth
      } catch (err) {
  ...
  useEffect(() => {
    // Lấy danh sách userId từ comments (chỉ lấy id dạng chuỗi dài, không phải tên)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 70px)' }}>
        <div style={{ width: 250, background: '#2c3e50', color: 'white' }}>
          <Sidebar />
        </div>
